...
function inject_tx_datasource() {
  ...
  if [ -n "${tx_backend}" ] ; then
    ...
    ## dummay $host $port added by ora01000@time-gate.com
    host="__"
    port="__"
...
#    fi
    ...
    driver="oracle"
    datasource="$(generate_tx_datasource ${service,,}-${prefix} $jndi $username $password $host $port $database $driver $oracle_connection_string)\n"
