    ...
    if [[ $f != "*.war" ]];then
      ...
      if [ -n "$web_xml" ]; then
        ...
        then
          ...
          jboss_web_xml=`read_web_dot_xml $f WEB-INF/jboss-web.xml`
...
function configure_client() {
  ...
  if [ -n "$result" ]; then
    echo "ERROR: Unable to registered $protocol client for module $module_name in realm $SSO_REALM on $redirects: $result"
  else
